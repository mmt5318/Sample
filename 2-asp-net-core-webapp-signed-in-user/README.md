...
cd active-directory-verifiable-credentials-dotnet/2-asp-net-core-webapp-signed-in-user
```
...
ngrok http 3000
```
2. Copy the HTTPS URL generated by ngrok in the `CallbackHost` part of the `appsettings.json`. 
![API Overview](ReadmeFiles/ngrok-url-screen.png)
The sample uses the `CallbackHost` to build the callback URL, this way the VC Request service can reach your sample web application to execute the callback method.
